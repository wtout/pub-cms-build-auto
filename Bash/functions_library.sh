...
function check_hosts_limit() {
	...
	then
		...
		[[ ${MYTAGS} =~ vm_creation|capcheck|infra_configure ]] && UPDATE_ARGS=1
		[[ ${MYTAGS} =~ infra_build_nodes ]] && UPDATE_ARGS=2
		if [[ ${UPDATE_ARGS} -eq 1 || ${UPDATE_ARGS} -eq 2 ]]
		then
			if [[ ! ${MYHOSTS} =~ dr ]]
			then
				VCENTERS='vcenter'
...
function send_notification() {
	...
	then
		...
		then
			...
			# shellcheck disable=SC2086
			$(docker_cmd) exec -e MYINVOKER="${MYINVOKER}" -t "${CNTNRNAME}" ansible-playbook playbooks/notify.yml --extra-vars "{SVCFILE: '${CONTAINERWD}/${SVCVAULT}', SNAME: '$(basename "${0}")', SARG: '${SCRIPT_ARG}', LFILE: '${CONTAINERWD}/${NEW_LOG_FILE}'}" --tags notify -e @"${SVCVAULT}" --vault-password-file Bash/get_common_vault_pass.sh -e @"${ANSIBLE_VARS}" ${NOTIF_ARGS} -v
		fi
