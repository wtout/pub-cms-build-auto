...
[[ "$(echo ${SCREDS_LIST})" != "" ]] && echo "${SCREDS_LIST[@]}" | sed "s/$(get_creds_prefix secondary)/S/g; s/^\(.*: \)\(.*\)$/\1'\2'/g" >> "${SVCVAULT}" || SCREDS_EMPTY=true
[[ ${PCREDS_EMPTY} && ${SCREDS_EMPTY} ]] && echo "Service creds not found" && Exit 1
[[ ${debug} == 1 ]] && set -x
